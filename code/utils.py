...
def get_fiber_channel_data(fiber_directory: Path) -> Dict[str, pd.DataFrame]:
    ...
    dict[str, pd.DataFrame]
        A dictionary, where each key is a channel,
        and the value is the corresponding csv read as a dataframe
    """
    fiber_data = {}
    for channel in FIBER_CHANNELS:
        ...
        if not channel_csv_path:
            raise FileNotFoundError(
                f"No {channel} csv found in "
                f"fiber directory path {fiber_directory}"
            )
